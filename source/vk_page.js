...
(function () {
   var PLUGIN_ID = 'linkshighlight';   //  Плагин преобразует ссылки на instagram.com, magnet и ftp из текста в собственно ссылки
   vkopt_plugins[PLUGIN_ID] = {
      ...
      processNode: function (node) {
         ...
         while (n = walk.nextNode()) {
            ...
            var regexps = [/magnet:\S+/, /ftp:\/\/\S*/];    // регулярки ссылок, которые уже частично подсвечиваются
            each(regexps, function () {
               var start = n.data.search(this);    // индекс начала ссылки
               if (start != -1) {
                  var url = this.exec(n.parentNode.textContent)[0];   // полный url, который надо заменить
