...
(function(){
    ...
    vkopt_plugins[PLUGIN_ID]={
        ...
        onLocation: function(nav_obj,cur_module_name){  // При переходе в новости добавить посты со стен
            if (getSet(20)=='y' && nav_obj[0]=="feed" && cur_module_name=="feed" && (!nav_obj.section || nav_obj.section=='news')) {
                var wall_list=ReadWallsCfg();
                this.later=[];
                for (var i in wall_list)
                    if (wall_list[i] != '')
                        this.loadWall(wall_list[i]);
            }
        },
        ...
        getPostTime: function (post) {  // Получение времени публикации сообщения по элементу post. Возвращается unixtime в GMT
            ...
            if (!time_element || hasClass(post.firstChild,'data-ad'))   // не пост или рекламный пост; возвращаем 1 января 3000 года (фактически значит "пропустить этот пост").
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (!hasClass(post, 'post_fixed') && !ge(post.id)) {    // закрепленные посты и уже существующие не обрабатываем
                    ...
                    if (j != feed_rows.length) {    // Если посту есть место в загруженной порции новостей
                        var feed_row = vkCe('div', {'class': 'feed_row'});
