...
var ex_ldr={ 
   ...
   get_scripts:function(callback){
      if (ext_browser.opera){                                                      // OPERA
         ...
         (chrome.extension.sendRequest || chrome.runtime.sendMessage)({act:'get_scripts', url:doc.location.href,in_frame:ex_ldr.is_in_frame(doc),key:ex_ldr.key}, function(data) {
            if (data.key==ex_ldr.key && data.files && data.files.length>0){
               if (data.api_enabled) 
                  api_enabled = true;
               callback(data.files);
         ...
         ex_api.post_message=function(msg){
            msg=ex_api.prepare_data(msg);
            (chrome.extension.sendRequest || chrome.runtime.sendMessage)(msg, function(data) {
               if (data.key==ex_ldr.key){
                  ex_api.message_handler(data);
               } 
            });
